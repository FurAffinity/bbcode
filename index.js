...
var unnestable = ['b', 'i', 's', 'u', 'url'];

var maximumStackSize = 20;
...
function transform(closes, token, startNode, endNode) {
	...
	switch (name) {
		...
		case 'url':
			...
			if (info.allowed) {
				startNode.text = '<a href="' + escapeAttributeValue(closes.token.value) + (info.internal ? '">' : '" rel="external nofollow">');
				endNode.text = '</a>';
				return true;
			}
	...
	for (var i = 0; i < tokens.length; i++) {
		...
		switch (token.type) {
			...
			case OPEN_TAG:
				...
				var redundant = unnestable.indexOf(name) !== -1 && openTagNames.indexOf(name) !== -1;

				var openingNode = append(escapeContent(token.text));
				openTags.push({ token: token, node: openingNode, closedOverBy: [], redundant: redundant });
				openTagNames.push(name);

				break;

			case CLOSE_TAG:
				var closesIndex = openTagNames.lastIndexOf(name);
			...
			case AUTOMATIC_LINK:
				...
				if (info.allowed) {
					append('<a href="' + escapeAttributeValue(token.text) + (info.internal ? '">' : '" rel="external nofollow">') + escapeContent(token.text) + '</a>');
				} else {
					append(escapeContent(token.text));
				}
